...
problemPSO = build_hologram_problem( ...
...
resultPSO = optimize_hologram_builtin(problemPSO, 'particleswarm', optsPSO);

figure;
subplot(1,3,1); imagesc(resultPSO.code_2d); axis image; colorbar; title('PSO编码矩阵');
subplot(1,3,2); imagesc(known_abs_spatial); axis image; colorbar; title('目标图像');
subplot(1,3,3); imagesc(resultPSO.target_amp_norm_2d); axis image; colorbar; title('PSO重建结果');

fprintf('PSO: score=%.6f, RMSE=%.6f, eta=%.6f, PCC=%.6f\n', ...
    resultPSO.score, resultPSO.rmse, resultPSO.eta_target, resultPSO.pcc);
%%
load('xianzai1.mat')
% yuanlai=B;
xianzai=resultPSO.target_amp_norm_2d;
...
PSO_matrix(:,1)=psnr_s;
PSO_matrix(:,2)=ssim_s;
PSO_matrix(:,3)=total_time_s;
save('PSO.mat','PSO_matrix')